<template>
    <div id="C016-01" class="container-fluid p-3 h-100 w-100">
        ...
        <div v-show="!isSelectedTarget">
            <div id="C016-01-03" class="container-fluid p-3 h-100 w-100 board">
            ...
            <div id="C016-01-01" class="container-fluid p-3 h-100 w-100 board" style="margin-top:20pt;">
            ...
            <div id="C016-01-02" class="container-fluid p-3 h-100 w-100 shadow-sm board" style="margin-top:20pt;">
                <div class="row">
                    <div class="px-2">
                        <div id="C016-01-02-01" class="text-left">
                            対象者選択
                        </div>
                    </div>
                </div>
                <div id="C016-01-02-02" class="card shadow h-100" style="background-color:#BCD2EE;">
                    <div class="card-body">
                        <div class="card-title">照会・承認対象者を選択してください</div>
        ...
        <div v-show="isSelectedTarget">
            <div id="C016-02-01" class="container-fluid px-3 h-100 w-100 shadow-sm board">
                <div class="row">
                    <div class="p-3 col-10 text-left">
                        <div id="C016-02-01-01" class="row">
                            <div style="font-size:11pt;">対象者選択</div>
                        </div>
                        <div class="row pl-5">
                    ...
                    <div class="p-3 col-2 text-center mt-3">
                        <button id="C016-02-01-04" style="font-size:11pt;width:100pt" class="btn btn-primary" v-on:click="exitInput()">照会・承認終了</button>
                    </div>
                </div>
            </div>
            <inputAttendance_board :employee_id="targetEmployeeID" :year_month="yearMonth" :is_manager="isManager" :background_type="backgroundType" :information_attendance_mode="informationAttendanceMode" :session_data="session_data" :is_selected_target="isSelectedTarget"></inputAttendance_board>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        initialize()
        {
            ...
            this.isManager = true;  //勤怠管理者メニューからリロードした時には「照会・承認」へ戻しておく
            this.informationAttendanceMode = 1; //勤怠管理者メニューからリロードした時には「照会・承認」へ戻しておく
        },
        getInputAgentList(){
            this.isLoading = true;
                ...
                if(response.data.result)
                {
                    ...
                    else
                    {
                        ...
                        for(let key in employee_array)
                        {
                            ...
                            if(employee_array[key].close_state_lastmonth == 2){
