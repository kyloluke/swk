<template>
    <div id="C017-01" class="container-fluid p-3 h-100 w-100">
        <laborManagementSearch_board :employee_id="employeeID" :is_selected_single="isSelectedSingle" @selectedList="selectedList"></laborManagementSearch_board>
        <div v-if="isSelectedList">
            <laborManagementList_board :list_id="listId" :is_selected_target="isSelectedTarget" :target_data_info="targetDataInfo" @selectTarget="selectTarget"></laborManagementList_board>
        </div>
        <div v-if="isSelectedTarget">
            <div id="C017-01-03" class="container-fluid p-3 h-100 w-100 shadow-sm board" style="margin-top:20pt;">
                <div class="row">
                    <div class="px-2">
                        <div id="C017-01-03-01" class="text-left">
                            対象者選択
                        </div>
                    </div>
                </div>
                <div class="row">
                    ...
                    <div class="px-3 col-9 text-left">
                        <div id="C017-01-03-04" style="font-size:15pt;color:black;line-height: 46px;">{{TextSelectTarget}}</div>
                    </div>
                    <div class="px-3 col-2 text-center">
                        <button id="C017-01-03-05" style="font-size:11pt;width:100pt" class="btn btn-primary" v-bind:disabled="!isSelectedTarget" v-on:click="exitInput()">対象者を閉じる</button>
                    </div>
                </div>
            </div>
