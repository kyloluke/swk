...
class t018_office_history extends Model
{
    use HasFactory;
    // テーブルの関連付け
    protected $table = 't018_office_history';
    // 更新可能な項目の設定
    protected $fillable = [
        'employee_id', 
        'office_id',
    ...
    protected $primaryKey = "office_history_id";
    ...
    {
        $t018OfficeHistory = DB::table($this->table)->get();

        return $t018OfficeHistory;
    }
    ...
    {
        return DB::table($this->table)
            ->select('office_history_id', 'office_id', 'office_code', 'office_name', 'office_short_name', 'valid_date_start', 'valid_date_end')
     ...
     * 過去のデータ更新
     */
    public function updatePreviousData($office_history_id, $update_start_serial, $update_end_serial, $userCode, $update_date)
    {
        DB::table($this->table)
            ->where('office_history_id', $office_history_id)
            ->update([
