...
class t021_post_history extends Model
{
    use HasFactory;
    // テーブルの関連付け
    protected $table = 't021_post_history';
    // 更新可能な項目の設定
    protected $fillable = [
        'employee_id', 
        'post_id',
    ...
    protected $primaryKey = "post_history_id";
     ...
     * 履歴情報新規
     */
    public function insertData($targetEmployeeID,$post_id,$post_code,$post_name,$post_short_name,$valid_date_start,$valid_date_end,$detail_no,$employeeCode,$date)
    {
    ...
    {
        return DB::table($this->table)
            ->select('post_history_id', 'post_id', 'post_code', 'post_name', 'post_short_name', 'valid_date_start', 'valid_date_end')
    ...
    {
        return DB::table($this->table)
            ->select('post_id', 'post_code', 'post_name', 'post_short_name', 'valid_date_start', 'valid_date_end')
            ->where('employee_id', $targetEmployeeID)
     ...
     * 過去のデータ更新
     */
    public function updatePreviousData($post_history_id, $update_start_serial, $update_end_serial, $userCode, $update_date)
    {
        DB::table($this->table)
            ->where('post_history_id', $post_history_id)
            ->update([
